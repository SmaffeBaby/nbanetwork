<template>
  <div class="space-y-4">
    ...
    <div v-if="loading" class="rounded-2xl border border-gray-100 bg-white px-3 py-10">
      <div
          role="status"
    ...
    <div v-else class="space-y-3 md:hidden">
      ...
      >
        <div class="flex items-center justify-between gap-3">
          <div class="text-sm font-medium text-gray-900">{{ game.FORMATTED_DATE }}</div>
          <div class="shrink-0 text-sm font-semibold text-gray-600">{{ game.MOSCOW_TIME }}</div>
        </div>

        <div class="mt-3 flex flex-wrap items-center gap-2">
          <img :src="getTeamLogo(parseMatchup(game.MATCHUP).away)" class="h-7 w-7 object-contain" alt="" />
          <span class="font-semibold">{{ parseMatchup(game.MATCHUP).away }}</span>

          <span class="text-xs text-gray-400">
            {{ parseMatchup(game.MATCHUP).isAway ? '@' : 'vs' }}
          </span>

          <img :src="getTeamLogo(parseMatchup(game.MATCHUP).home)" class="h-7 w-7 object-contain" alt="" />
          <span class="font-semibold">{{ parseMatchup(game.MATCHUP).home }}</span>
        </div>
      </div>
    </div>
        ...
        >
          ...
          <td class="px-3 py-2">
            <div class="flex items-center gap-2">
              <img :src="getTeamLogo(parseMatchup(game.MATCHUP).away)" class="w-6 h-6" alt="" />
              ...
              <img :src="getTeamLogo(parseMatchup(game.MATCHUP).home)" class="w-6 h-6" alt="" />
